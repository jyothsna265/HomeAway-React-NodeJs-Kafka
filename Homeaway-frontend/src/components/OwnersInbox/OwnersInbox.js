...
class OwnersInbox extends Component {
    ...
    constructor(){
      ...
      componentDidMount(){
        axios.get('http://localhost:3001/OwnersInbox')
                .then((response) => {
    ...
    render(){  
        let travMessage = this.state.message.map(message => {
            return (
                <OwnersInboxComponent>{message.message}{message.emailID}{message.firstName}{message.lastName}{message.msgID}</OwnersInboxComponent>
            )
        })
